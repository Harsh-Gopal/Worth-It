  ...
  return (
    <div className="flex flex-wrap items-center gap-2 p-2 bg-[var(--bg-main)] border border-[var(--border-color)] rounded-xl focus-within:ring-1 focus-within:ring-[var(--color-brand-green)] focus-within:border-[var(--color-brand-green)] transition-all min-h-[46px]">
      {keywords.map(keyword => (
        <span key={keyword} className="flex items-center gap-1 bg-[var(--bg-surface-hover)] border border-[var(--border-color)] text-white text-sm px-2.5 py-1 rounded-md shadow-[0_2px_5px_rgba(0,0,0,0.5)]">
          {keyword}
      ...
      <input
        ...
        className="flex-1 min-w-[150px] bg-transparent border-none focus:ring-0 text-sm text-white placeholder:text-[var(--text-secondary)] py-1 px-1 outline-none"
      />
    </div>
  );
}
